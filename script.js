...
// Skills section observer - triggers all skill bars with staggered animation every time it enters the viewport
    ...
    entries.forEach(entry => {
        ...
        if (entry.isIntersecting) {
            skillBars.forEach((bar, index) => {
                ...
                // Ensure starting from 0 for animation effect
                bar.style.width = '0%';
                setTimeout(() => {
            ...
            // Reset widths to 0 when scrolling away, so the animation plays again next time
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Observe the skills section as a whole for bar animations
...
function updateActiveNavLink() {
    ...
    sections.forEach(section => {
        ...
        // A section is active if the vertical center of the screen lies within its boundaries
        if (rect.top <= viewportCenter && rect.bottom >= viewportCenter) {
            current = section.getAttribute('id');
        }
    });

    // Fallback: scrolled near the very bottom of the page -> highlight Contact
    if ((window.innerHeight + window.scrollY) >= document.documentElement.scrollHeight - 60) {
        current = 'contact';
    }
    
    // Fallback: scrolled near the very top of the page -> highlight Home
...
// Also run on load so the correct link is highlighted on page load
